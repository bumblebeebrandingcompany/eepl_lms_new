...
class BookingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.booking_new.index');
    }
    ...
    public function getLeadDetails(Request $request)
    {
        if ($request->ajax()) {
            ...
            $leadsQuery = Lead::where(function ($query) use ($searchTerm) {
                $query->whereJsonContains('essential_fields->Phone_Number', 'like', '%' . $searchTerm . '%')
                      ->orWhereJsonContains('essential_fields->Addl_Number', 'like', '%' . $searchTerm . '%');
            });
